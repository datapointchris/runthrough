...
def produce(
    tape: dict, question: str, library: Path, confirm: bool, watch: bool
) -> Path:
...
def play(
    ...
    out: Annotated[
        Path | None, typer.Option("--out", help="Where to write the page.")
    ] = None,
    cast: Annotated[
        Path | None, typer.Option("--cast", help="Also write an asciicast v2 file.")
    ] = None,
    fps: Annotated[int, typer.Option("--fps", help="Screen samples per second.")] = 14,
    watch: Annotated[
        bool, typer.Option("--watch/--no-watch", help="Open the page when done.")
    ] = True,
) -> None:
...
def ask(
    question: Annotated[
        list[str], typer.Argument(help="What you want shown, in English.")
    ],
    cli: Annotated[str, typer.Option("--cli", help="Which command to demonstrate.")],
    sandbox: Annotated[
        bool, typer.Option("--sandbox", help="Redirect XDG roots to a throwaway dir.")
    ] = False,
    yes: Annotated[
        bool, typer.Option("--yes", help="Record without confirming the tape.")
    ] = False,
    budget: Annotated[
        int, typer.Option("--budget", help="Help-text characters to read.")
    ] = 40000,
    library: Annotated[
        Path, typer.Option("--library", help="Where tapes are kept.")
    ] = DEFAULT_LIBRARY,
    watch: Annotated[
        bool, typer.Option("--watch/--no-watch", help="Open the page when done.")
    ] = True,
...
def fleet(
    question: Annotated[
        list[str], typer.Argument(help="What you want answered, in English.")
    ],
    yes: Annotated[
        bool, typer.Option("--yes", help="Record without confirming the tape.")
    ] = False,
    ...
    for path in paths:
        ...
        allowed.add(parts[0])
        text = (
            ask_module.gather_help(parts[0], budget // max(1, len(paths)))
            if len(parts) == 1
            else ask_module.run_help(parts)
        )
        if text.strip():
